...
function Sparkles({ color = '#ff8a3d', density = 80 }) {
...
function SoftwareChip({ sw }) {
  return (
    <div style={{
      ...
      onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,138,61,0.35)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
  ...
  return (
    <section id="software" style={{ padding: '8rem 0', borderTop: '1px solid hsl(var(--stroke))', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', height: 120, marginTop: '1rem', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 100%, rgba(255,138,61,0.12), transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '80%', height: 1, background: 'linear-gradient(90deg, transparent, rgba(255,138,61,0.3), transparent)' }} />
      </div>
    </section>
  );
}

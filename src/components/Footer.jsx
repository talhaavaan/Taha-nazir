...
export default function Footer() {
  ...
  const sendWhatsApp = () => {
    if (isEmpty) return;
    const text = `Hi Taha! I'm ${form.name}${form.email ? ` (${form.email})` : ''}.\n\n${form.message}`;
  ...
  const focusStyle = (name) => focusedField === name
    ? { borderColor: 'rgba(255,138,61,0.5)', background: 'rgba(255,138,61,0.05)' }
    : {};
    ...
    <footer style={{ position: 'relative', overflow: 'hidden', paddingTop: '8rem', paddingBottom: '3rem', borderTop: '1px solid hsl(var(--stroke))' }}>
      ...
      <div style={{ position: 'relative', zIndex: 10 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginTop: '.25rem' }}>
              ...
              <button
                ...
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '.5rem',
                  padding: '.85rem 1.2rem', borderRadius: 9999,
                  background: sent === 'email' ? 'rgba(255,138,61,0.15)' : 'rgba(255,138,61,0.06)',
                  border: `1px solid ${sent === 'email' ? 'rgba(255,138,61,0.5)' : 'hsl(var(--stroke))'}`,
                  color: sent === 'email' ? '#FF8A3D' : 'hsl(var(--text))',
                  fontFamily: "'Inter',sans-serif", fontSize: '.82rem', fontWeight: 500,
                  cursor: isEmpty ? 'not-allowed' : 'pointer',
                  opacity: isEmpty ? 0.4 : 1,
                  transition: 'all .2s',
                ...
                onMouseEnter={e => { if (!isEmpty) { e.currentTarget.style.borderColor = 'rgba(255,138,61,0.4)'; e.currentTarget.style.background = 'rgba(255,138,61,0.1)'; } }}
                onMouseLeave={e => { if (!isEmpty && sent !== 'email') { e.currentTarget.style.borderColor = 'hsl(var(--stroke))'; e.currentTarget.style.background = 'rgba(255,138,61,0.06)'; } }}
              >
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1.5rem' }}>
          <div>
            <div style={{ fontFamily: "'Instrument Serif',serif", fontStyle: 'italic', fontSize: '1.2rem', letterSpacing: '.02em', background: 'linear-gradient(90deg,var(--accent-from),var(--accent-to))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              Taha Nazir
            </div>
            <div style={{ color: 'hsl(var(--muted))', fontSize: '.72rem', marginTop: '.25rem', fontFamily: "'Inter',sans-serif" }}>Video Editor · Digital Content Creator</div>
          </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '3rem', color: 'hsl(var(--muted))', fontSize: '.72rem', fontFamily: "'Inter',sans-serif" }}>
          © 2026 Taha Nazir. All rights reserved.
        </div>
